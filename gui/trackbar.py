...
class TrackbarSecondary(object):
	...
	def setdefault(self, index):
		...
		cv2.createTrackbar('Win Height %s' % str(index), variables.app_window_trackbar_name_2,
						   self.background.win_height, 100, self.setwinheight)
		cv2.createTrackbar('Win Width %s' % str(index), variables.app_window_trackbar_name_2,
						   self.background.win_width, 100, self.setwinwidth)
		cv2.createTrackbar('Min Pix Win %s' % str(index), variables.app_window_trackbar_name_2,
